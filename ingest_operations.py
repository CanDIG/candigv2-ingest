...
def list_role(role_type):
    try:
        result, status_code = auth.get_role_type_in_opa(role_type)
...
def update_role(role_type):
    ...
    try:
        result, status_code = auth.set_role_type_in_opa(role_type, role_members)
        return result, status_code
    except Exception as e:
        return {"error": str(e)}, 500
...
def add_user_to_role(role_type, email):
    try:
        ...
        if status_code == 200:
            ...
            result, status_code = auth.set_role_type_in_opa(role_type, result[role_type])
        return result, status_code
    except Exception as e:
        return {"error": str(e)}, 500
