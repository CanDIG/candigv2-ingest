    ...
    try:
        ...
        if errors:
            raise ValidationError("Clinical etl conversion failed to create an ingestable json file, "
                                  "please check the errors in tests/clinical_data_validation_results.json and try again.")
    except Exception as e:
        print(e)
        print(f"Moving validation results file to {ingest_repo_dir}/tests/clinical_data_validation_results.json.")
        shutil.move(f"{args.output}/small_dataset_csv/raw_data_validation_results.json",
                    f"{ingest_repo_dir}/tests/clinical_data_validation_results.json")
        print("Removing repo.")
